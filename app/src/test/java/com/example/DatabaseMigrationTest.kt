...
class DatabaseMigrationTest {
    ...
    fun `every legacy version retains rows through version eight`() = runBlocking {
        ...
        for (version in 1..7) {
            ...
            try {
                ...
                assertEquals("Pop", session.genre)
                assertEquals(2220L, session.durationSeconds)
                assertEquals(0, migrated.openHelper.readableDatabase.query("SELECT COUNT(*) FROM resolved_genres").use { it.moveToFirst(); it.getInt(0) })
            } finally {
                migrated.close()
    ...
    private fun createLegacyDatabase(context: Context, name: String, version: Int) {
        ...
        SQLiteDatabase.openOrCreateDatabase(path, null).use { database ->
            ...
            val base = "id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, date TEXT NOT NULL, year INTEGER NOT NULL, month INTEGER NOT NULL, startTime INTEGER NOT NULL, endTime INTEGER NOT NULL, durationSeconds INTEGER NOT NULL, title TEXT, artist TEXT, album TEXT, genre TEXT, artworkUrl TEXT, sourcePackage TEXT NOT NULL"
            val extra = (if (version >= 4) ", playCount INTEGER NOT NULL DEFAULT 1" else "") +
                (if (version >= 5) ", dailyDurations TEXT, isOpen INTEGER NOT NULL DEFAULT 0" else "")
            database.execSQL("CREATE TABLE playback_sessions ($base$extra)")
            if (version >= 6) {
