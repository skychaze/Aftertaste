...
The live card at the top of the main screen. Shows title, artist, genre, artwork, a running session timer, and a position/duration timeline while YouTube Music plays. One tap opens YT Music (web fallback to music.youtube.com). A permission banner appears when notification listener access is missing. The top-right App Info action opens the "How YT Track Works" dialog.
...
Cold-start the app. The card is the first thing under the header, no navigation needed. Tap the top-right `content-desc` "App Info" action to read the help dialog, then dismiss it with "Got It".

## Driving it with adb

1. Launch. Exercise the help dialog with "App Info" and "Got It" if it is part of this drive.
...
- The seeder's sessions carry `sourcePackage=com.google.android.apps.youtube.music`, so a `sourcePackage LIKE '%youtube%music%'` query cannot distinguish seeded from real sessions. Seeding is a no-op once the selected year already has daily stats or sessions; distinguish an existing seed from live playback by exact track totals from the catalog (for example, Levitating 2772s or Do I Wanna Know? 1848s).
